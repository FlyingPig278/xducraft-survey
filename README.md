...
### 可选方案：Vercel 前端 + 自托管后端

如果希望前端走 Vercel CDN，可以这样拆：

1. 前端部署到 Vercel。
2. 后端部署在自家公网 IPv4 服务器。
3. Vercel 环境变量设置：

```bash
VITE_API_BASE_URL=https://api.xducraft.cn
```
...
```bash
FRONTEND_BASE_URL=https://vote.xducraft.cn
BLESSING_REDIRECT_URI=https://api.xducraft.cn/api/auth/blessing/callback
```

这种方案比较方便，但要处理跨域、HTTPS、反代和两个域名的配置。
